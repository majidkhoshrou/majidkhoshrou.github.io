...
from typing import List, Dict, Iterator, Set, Optional
...
def sentence_tokenize(text: str) -> List[str]:
    """Tokenize text into sentences using spaCy (fallback to raw text)."""
    try:
...
def split_text(text: str, chunk_size: int = 500, overlap: int = 50) -> Iterator[str]:
    """Split long text into overlapping sentence-aware word chunks."""
        ...
        for sentence in sentences:
            ...
            if current_word_count + word_count <= chunk_size:
                ...
                if current_chunk:
                    ...
                    if overlap > 0:
                        current_chunk = current_chunk[-1 * (overlap // 10):]
...
def extract_all_body_text(soup: BeautifulSoup) -> str:
    """Extract readable text from the <body> of HTML."""
    if not soup.body:
        return ""
    texts = []
...
def extract_title_from_html(soup: BeautifulSoup) -> str:
    """Extract the HTML <title> or first <h1>."""
...
def extract_first_level_html_links(
    ...
    """
    Follows internal/external HTML-like links up to max_depth and extracts chunks per source.
    Each linked HTML is treated as a separate chunked document.
    """
    ...
    for tag in soup.find_all("a", href=True):
        ...
        if href.startswith("http"):
            ...
            try:
                ...
                linked_html = response.text
                linked_soup = BeautifulSoup(linked_html, "html.parser")
                title = extract_title_from_html(linked_soup)
                body_text = extract_all_body_text(linked_soup)
                sub_chunks = extract_first_level_html_links(
                    base_path, linked_soup, seen_links, chunk_size, overlap, max_depth, current_depth + 1
                )
                ...
                chunks = [
                    ...
                    for i, chunk in enumerate(split_text(full_text, chunk_size, overlap))
                    if len(chunk.strip().split()) > 10
                ]
                results.extend(chunks + sub_chunks)
            except Exception as e:
                logger.warning(f"Skipping external link {href}: {e}")

        # Internal (relative) link
        else:
            linked_path = (base_path.parent / href).resolve()
            key = str(linked_path)
            if key in seen_links or not linked_path.exists():
                continue
            seen_links.add(key)
            try:
                ...
                body_text = extract_all_body_text(linked_soup)
                sub_chunks = extract_first_level_html_links(
                    linked_path, linked_soup, seen_links, chunk_size, overlap, max_depth, current_depth + 1
                )
                full_text = clean_text(body_text)
                timestamp = datetime.utcnow().isoformat()
                ...
                chunks = [
                    {
                        "id": f"html_{linked_path.stem}_{i}",
                        "source": str(linked_path.name),
                        "type": "html",
                        "chunk_index": i,
                        "title": title,
                        "timestamp": timestamp,
...
def process_html_file(
    ...
    """
    Processes a base HTML file and optionally its linked pages, returning all chunks with traceable sources.
    """
    ...
    try:
        ...
        # Follow links (external and internal)
        if follow_links:
            linked_chunks = extract_first_level_html_links(
                path, soup, seen_links, chunk_size, overlap, max_depth
            )
...
def extract_first_level_pdf_links(doc: fitz.Document, base_path: Path, seen_links: Set[str]) -> str:
    """Extract embedded PDF links (local only) and merge their text."""
    texts = []
    for page in doc:
...
def process_pdf_file(
    ...
    """
    Extract structured text chunks from a single PDF file.
    """
    seen_links = seen_links or set()
    try:
...
def save_chunks(chunks: List[Dict[str, str]], output_path: Path) -> None:
    """
    Save extracted chunks as a JSON file to disk.
    """
    try:
